...
export default function BottomSheet({ children }: { children: ReactNode }) {
  ...
  return (
    ...
    >
      {/* Big, full-width grab bar — easy to hit on touch without scrolling. */}
      <div
        ...
        className="flex min-h-[48px] shrink-0 cursor-grab touch-none select-none items-center justify-center active:cursor-grabbing"
      >
        <div className="h-1.5 w-12 rounded-full bg-slate-300" />
      </div>
      {/* Only scroll when expanded so a missed grab on the peek can't scroll it. */}
      <div
        className={`min-h-0 flex-1 overscroll-contain px-5 pb-[max(20px,env(safe-area-inset-bottom))] ${
